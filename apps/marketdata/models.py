...
class value_scheme(models.Model):
    ...
    value_scheme_pref = models.ForeignKey(
        value_scheme_pref, on_delete=models.PROTECT, related_name='value_scheme_value_scheme_pref'
    )
    ...
    owner_org = models.ForeignKey(
        'staticdata.organization', on_delete=models.PROTECT, related_name='value_scheme_owner'
    )
    created = models.DateTimeField(default=now, blank=True, unique=False)
    updated = models.DateTimeField(default=now, blank=True, unique=False)
...
class value_scheme_rule(models.Model):
    name = models.CharField(max_length=50, unique=True)
    instrument_type_id = models.ForeignKey(
        'classifiers.instrument_class', on_delete=models.PROTECT, related_name='value_scheme_rule_instrument_type'
    )
    instrument_class_id = models.ForeignKey(
        'classifiers.instrument_class',
        blank=True,
        null=True,
    ...
    book = models.ForeignKey(
        'accounting.book', blank=True, null=True, on_delete=models.CASCADE, related_name='value_scheme_rule_book'
    )
    account = models.ForeignKey(
        'accounting.broker_account',
        blank=True,
        null=True,
...
class xrate_ladder(models.Model):
    value_date = models.DateField(blank=False, null=False, unique=False)
    instrument = models.ForeignKey('staticdata.instrument', on_delete=models.CASCADE)
    bid_xrate = models.FloatField(
        null=False,
        blank=False,
        unique=False,
        validators=[MinValueValidator(0.0)],
    )
    ask_xrate = models.FloatField(
        null=False,
        blank=False,
        unique=False,
        validators=[MinValueValidator(0.0)],
