    ...
    if request.method == 'GET':
        if id == None:  # list of all trades requested
            try:
                ...
                status = 'OK'
            return JsonResponse({'status': status, 'data': {"trades": data}}, safe=False)
        ...
        if trade_data['trade_ccy'] == trade_data['settle_ccy']:
            ...
            return JsonResponse(
                {
                    "data": 'FX rate needs to be specified for different trade and settlement currencies',
                    "status": 'NOK',
                },
                safe=False,
            )
        ...
        if trade_data['settle_ccy'] != base_ccy:
            if 'settle_base_xrate' not in trade_data:
                return JsonResponse(
                    {
                        "data": 'FX rate needs to be specified for different settlement and base currencies',
                        "status": 'NOK',
                    },
                    safe=False,
                )
        ...
        if not get_org_by_name_and_type(trade_data['counterparty'], org_type='Counterparty'):
            get_or_save_organization(
                org_type='Counterparty', name=trade_data['counterparty'], description='New trade counterparty'
            )
